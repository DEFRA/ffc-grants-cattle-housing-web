...
it('user selects eligible option: \'floorSpace: 22\' -> display environmental-impact', async () => {
  ...
  it('should return an error message if number contains a comma "," ', async () => {
    const postOptions = {
        method: 'POST',
        url: `${global.__URLPREFIX__}/floor-space-over150kg`,
        payload: { floorSpace: '123,45', crumb: crumbToken },
        headers: { cookie: 'crumb=' + crumbToken }
    }
  ...
  it('should return an error message if the number of characters exceed 5', async () => {
    const postOptions = {
        method: 'POST',
        url: `${global.__URLPREFIX__}/floor-space-over150kg`,
    ...
    const postResponse = await global.__SERVER__.inject(postOptions)
    expect(postResponse.statusCode).toBe(200)
    expect(postResponse.payload).toContain('Number must be between 5-99.99')
  })
