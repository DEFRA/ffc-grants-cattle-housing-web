...
describe('Project Summary test', () => {
  ...
  test('POST /project-summary route returns next page on continue', async () => {
    ...
    expect(response.headers.location).toBe('/upgrading-cattle-housing/potential-amount')
  })
  ...
  test('POST /project-summary route returns next page \'on change your items\'', async () => {
    ...
    const response = await global.__SERVER__.inject(options)
    expect(response.statusCode).toBe(302)
    expect(response.headers.location).toBe('/upgrading-cattle-housing/storage-type')
  })
  ...
  test('page loads with correct back link', async () => {
    ...
    const response = await global.__SERVER__.inject(options)
    expect(response.statusCode).toBe(200)
    expect(response.payload).toContain('<a href=\"/upgrading-cattle-housing/item-sizes-quantities\" class=\"govuk-back-link\">Back</a>')
  })
